...
\begin{align*}
  \text{IG}(x, y) &= H\big[p(S)\big] - H\big[p(S \mid x, y)\big] \\[-0.3ex]
  \text{EIG}(x) &= \mathbb{E}_{y \sim p(y \mid x)}\!\big[\text{IG}(x, y)\big], \quad
  x^{\star} = \arg\max_{x \in X}\; \text{EIG}(x)
\end{align*}

\vspace{-2ex}

For binary answers with a noisy oracle ($\text{BSC}(\varepsilon)$):
%
\begin{equation*}
  \boxed{\;\text{EIG}_\varepsilon(S;\, q) = H_b\!\big(\varepsilon + (1 - 2\varepsilon)\,p\big) - H_b(\varepsilon)\;}
\end{equation*}
...
\heading{$Q_{\text{Bayes}}$: Maximize information gain}
\vspace{-1.5ex}
Sample $K$ candidates from the LM, select the most informative:
\vspace{-0.5ex}
\begin{equation*}
  q_t^{\star} \in \arg\max_{q \in \mathcal{Q}}\; \text{EIG}_\varepsilon(q \mid b_t)
\end{equation*}

\vspace{-2ex}

\heading{$M_{\text{Bayes}}$: Maximize hit probability}
\vspace{-1.5ex}
Select the tile most likely to contain a ship:
\vspace{-0.5ex}
\begin{equation*}
  u_t^{\star} \in \arg\max_{u}\; p_{\text{hit}}(u \mid b_t), \;\; p_{\text{hit}}(u \mid b_t) \coloneqq \!\!\sum_{s \in \mathcal{F}} b_t(s)\, \mathbb{1}[u \text{ is ship in } s]
\end{equation*}

\vspace{-2ex}

\heading{$D_{\text{Bayes}}$: Explore vs.\ exploit}
\vspace{-1.5ex}
One-step lookahead---ask if expected post-question hit prob.\ exceeds current:
\vspace{-0.5ex}
\begin{equation*}
  \text{Ask } q_t^{\star} \;\text{if}\; \gamma\,\widehat{p}_{\text{hit}}^{\,t+1}(q_t^{\star}) \;>\; p_{\text{hit}}(u_t^{\star} \mid b_t)
\end{equation*}
...
\small
[1] Lindley (1956). \textit{Ann.\ Math.\ Stat.} \enspace
[2] Rothe et al.\ (2018). \textit{CBB.} \enspace
[3] Wong, Grand et al.\ (2023). \textit{arXiv}.
