...
def grow_ship(partial_board: Board, ship_picked, length_picked, max_length_dict):
        ...
        if ship_picked.length == 1:
            allowable_orientations = [orientation for orientation in ship_picked.orientation if max_length_dict[ship_picked.label][orientation] >= length_picked]
        ...
        if ship_picked.orientation == "H":
            ...
            while np.count_nonzero(row == ship_picked.label) < length_picked: #keep first steady, change second
                ship_picked = SeenShip(Board.from_symbolic_array(partial_board), ship_picked.label)
                valid_expansions = []
                left_extremity, right_extremity = ship_picked.extremities[0], ship_picked.extremities[1]
                if left_extremity[1] - 1 >= 0 and partial_board[left_extremity[0]][left_extremity[1]-1] == "H":
                    valid_expansions.append((left_extremity[0], left_extremity[1] - 1))
                if right_extremity[1] + 1 < len(row) and partial_board[right_extremity[0]][right_extremity[1]+1] == "H":
        ...
        if ship_picked.orientation == "V":
            ...
            while np.count_nonzero(column == ship_picked.label) < length_picked: #keep second steady, change first
                ship_picked = SeenShip(Board.from_symbolic_array(partial_board), ship_picked.label)
                valid_expansions = []
                up_extremity, down_extremity = ship_picked.extremities[0], ship_picked.extremities[1]
                if down_extremity[0] + 1 < len(column) and partial_board[down_extremity[0]+1][down_extremity[1]] == "H":
                    valid_expansions.append((down_extremity[0] + 1, down_extremity[1]))
                if up_extremity[0] - 1 >= 0 and partial_board[up_extremity[0]-1][up_extremity[1]] == "H":
...
def sample_board(partial_board: Board, ship_labels, ship_lengths):
    ...
    partial_board = seed_board(partial_board, [ship.label for ship in temp_ships if ship.length == 0])
    ...
    while any([assigned_length == None for assigned_length in assigned_length_dict.values()]):

        if partial_board is None:
            return None

        unassigned_lengths = [length for length in ship_lengths if length not in assigned_length_dict.values()]
        current_length_dict = {ship.label: ship.length for ship in ships}
        max_length_dict = {ship.label: 
                                {
                                "V": np.count_nonzero(partial_board[:,ship.extremities[0][1]] == ship.label) + np.count_nonzero(partial_board[:,ship.extremities[0][1]] == "H"),
                                "H": np.count_nonzero(partial_board[ship.extremities[0][0],:] == ship.label) + np.count_nonzero(partial_board[ship.extremities[0][0],:] == "H")
                                } 
                                for ship in ships}
        possible_length_dict = {ship.label: [length for length in unassigned_lengths if length >= current_length_dict[ship.label] and length <= max([max_length_dict[ship.label][orient] for orient in ship.orientation])] for ship in ships}
        forced_unassigned_ships = {ship_label: ship_lengths[0] for ship_label, ship_lengths in possible_length_dict.items() if len(ship_lengths) == 1 and assigned_length_dict[ship_label] == None}
        if len(forced_unassigned_ships.keys()) != 0:
            ...
            if ship.length != forced_ship[1]:
                partial_board = grow_ship(partial_board, ship, forced_ship[1], max_length_dict)
            else:
                pass
                #print(f"[SB] ship already of desired length, moving on")
            continue
        else:
            unforced_unassigned_ships = [ship.label for ship in ships if assigned_length_dict[ship.label] == None]
            try:
            ...
            ship_picked = SeenShip(Board.from_symbolic_array(partial_board), ship_label_picked)
            ...
            if ship_picked.length != length_picked:
                partial_board = grow_ship(partial_board, ship_picked, length_picked, max_length_dict)
            else:
                pass
            #    print(f"[SB] ship already of desired length, moving on")
